...
drawbar(Monitor *m)
{
	int x, xx, w;//, dx; no squares
	unsigned int i, occ = 0, urg = 0;
	Client *c;

	//dx = (drw->fonts[0]->ascent + drw->fonts[0]->descent + 2) / 4; no squares
	...
	for (i = 0; i < LENGTH(tags); i++) {
		...
		//drw_rect(drw, x + 1, 1, dx, dx, m == selmon && selmon->sel && selmon->sel->tags & 1 << i, no squares
		           //occ & 1 << i, 0); no squares
		x += w;
	}
